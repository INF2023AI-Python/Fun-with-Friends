...
class Scoreboard:
    ...
    def draw(self):
        ...
        graphics.DrawText(offscreen_canvas, font, 1, SCREEN_HEIGHT - 6, pointsColor, str(self.player1_points))

        # Draw player 2's points on the right side
        graphics.DrawText(offscreen_canvas, font, SCREEN_WIDTH - 9, SCREEN_HEIGHT - 6, pointsColor, str(self.player2_points))

        # Draw remaining time in the middle
        remaining_seconds = self.remaining_time % 60
        time_text = f"{remaining_seconds:02d}"
        graphics.DrawText(offscreen_canvas, font, SCREEN_WIDTH // 2 - len(time_text) // 2, SCREEN_HEIGHT - 6, timeColor, time_text)
